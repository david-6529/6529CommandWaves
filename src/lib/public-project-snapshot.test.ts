...
describe("public project snapshot", () => {
  it("summarizes current hook work without implying the placeholder repo can run code", () => {
    ...
    expect(snapshot).toMatchObject({
      summary:
        "This page is the working snapshot for the 6529 AMM hook build. Builders use chat to shape ideas, record decisions, attach pull requests, and review the result. Current focus: Draft the non-upgradeable hook scaffold. Next step: Select the GitHub repo before PR work can run. The GitHub repo is not selected yet, so PR work waits.",
